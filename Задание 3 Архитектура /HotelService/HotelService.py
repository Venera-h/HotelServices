...
def cancel_reservation(order_id: str):
    ...
    Освобождает номер и публикует PaymentFailed,
    чтобы OrderService обновил статус заказа на CANCELLED.
    """
...
def handle_order_created(event):
    ...
    Если есть свободные номера — бронируем и публикуем HotelReserved.
    Если нет — публикуем HotelReservationFailed (компенсация).
    """
    hotel_id = event["hotel_id"]
    order_id = event["order_id"]
    if hotel_inventory.get(hotel_id, {}).get("rooms", 0) > 0:
        hotel_inventory[hotel_id]["rooms"] -= 1
        reservations[order_id] = hotel_id
        print(f"[HotelService] Номер забронирован для заказа {order_id}.")
        publish("HotelReserved", event)
    else:
        print(f"[HotelService] Нет свободных номеров в отеле {hotel_id}.")
        publish("HotelReservationFailed", {"order_id": order_id})
