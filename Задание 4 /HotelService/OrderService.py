...
def create_order(order_id, hotel_id, amount):
    ...
    Outbox pattern: атомарно пишем в orders и кладём событие OrderCreated в outbox.
    Relay-поток опубликует его в Kafka.
    """
    print(f"[OrderService] Заказ {order_id} создан (PENDING).")
    save(
        db_write=lambda: orders.update({order_id: "PENDING"}),
        topic="OrderCreated",
        payload={"order_id": order_id, "hotel_id": hotel_id, "amount": amount},
    )
